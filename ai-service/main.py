...
async def log_provider_status():
    ...
    for name in ["gemini", "groq", "ollama", "openai"]:
        provider = provider_factory.providers.get(name)
        if provider is None:
            print(f"[AI]   {name}: not registered")
            continue
        try:
            available = provider.is_available()
        except Exception:
            available = False
        print(f"[AI]   {name}: {'configured' if available else 'not configured / unreachable'}")
    priority = os.getenv("AI_PROVIDER_PRIORITY", "gemini,groq,ollama")
    print(f"[AI] Fallback order: {priority}")
